...
circleMine = new Layer
	...
	rotation: -90
	y: 64
circleMine.pathLength = 2*Math.PI*radius
circleMine.html = """
  <svg viewBox='-#{stroke/2} -#{stroke/2} #{viewBox} #{viewBox}'>
    <circleMine fill='none' stroke='#ffffff' stroke-linecap = 'round'
      stroke-width      = '#{stroke}'
      stroke-dasharray  = '#{circleMine.pathLength}'
      stroke-dashoffset = '0'
      cx = '#{radius}'
      cy = '#{radius}'
      r  = '#{radius}'>
...
circleMine.centerX() 
Utils.domComplete ->
 	circleMine.path = document.querySelector('svg circleMine')
 	
circleMine.visible = false
...
scrollMine.on Events.Move, ->
	...
	circleMine.visible = true
	
scrollMinetoY = (y) ->
	offset = Utils.modulate(y, [0, -96],[circleMine.pathLength, 0],true)
	circleMine.path.setAttribute 'stroke-dashoffset', offset
...
content1 = new Layer
	...
	backgroundColor: null
scroll = ScrollComponent.wrap(content1)
scroll.parent = view_invest
scroll.scrollHorizontal = false
scroll.speedY = 0.5
# scroll.superLayer = view_invest
scroll.content.placeBehind(tabBar)
...
scrolltoY = (y) ->
	for i in [amounts-4...amounts]
		scroll.content.children[0].children[i].children[9].width = 0
		scroll.content.children[1].children[i].children[9].width = 0
		scroll.content.children[2].children[i].children[9].width = 0
		
		scroll.content.children[0].children[i].children[9].width = Utils.modulate(y,[252*i-917,252*(i+1)-917],[0,358],true)
		scroll.content.children[1].children[i].children[9].width = Utils.modulate(y,[252*i-917,252*(i+1)-917],[0,358],true)
